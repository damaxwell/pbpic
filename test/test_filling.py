...
def TestFillColorGSave():
  ...
  pbp.setlinewidth(0.2*cm)
  pbp.build(paths.circle,0.5,at=(0,0))
  with pbp.gsave():
    pbp.build(paths.circle,0.8,at=(0,0))
...
def TestClip():
  ...
  pbp.build(paths.circle,1.5,at=(0,0))
  pbp.stroke(color.red)

  pbp.build(paths.circle,1.5,at=(0,0))
  pbp.clip()
  pbp.build(paths.square,L=2,at=(1,1))
  pbp.fill(color.blue)
